...
/**
 * Independent VXP compatibility layer compositor implemented from observed guest behavior.
 *
...
class MreGraphics(
    ...
    /** Move an existing layer without touching its guest pixel storage. */
    fun translateLayer(handle: Int, x: Int, y: Int): Int {
    ...
    /** Return the guest RGB565 buffer for either a loaded image/canvas or a layer. */
    fun getImageBuffer(handle: Int): Int =
        canvases[handle]?.bufferAddress ?: layers[handle]?.bufferAddress ?: 0

    /** Images loaded by this runtime are canvas-backed, so releasing an image releases that canvas. */
    fun releaseImage(handle: Int): Int = releaseCanvas(handle)

    /** Change transparent color on either a layer or canvas. */
    fun setTransparentColor(handle: Int, color: Int): Int {
        val c = color and 0xFFFF
        canvases[handle]?.let { it.transparentColor = c; return 0 }
        layers[handle]?.let { it.transparentColor = c; return 0 }
        return -1
    }

    /**
     * In-place software mirror for guest-owned RGB565 surfaces.
     * mode bit 0 = horizontal, bit 1 = vertical. Unknown/zero mode is a no-op.
     * This is deliberately handle-based so no host memory is exposed.
     */
    fun mirror(handle: Int, mode: Int): Int {
        val surface = when {
            layers.containsKey(handle) -> layers.getValue(handle).let { SurfaceRef(it.bufferAddress, it.width, it.height, it.clip) }
            canvases.containsKey(handle) -> canvases.getValue(handle).let { SurfaceRef(it.bufferAddress, it.width, it.height, null) }
            else -> return -1
        }
        val horizontal = (mode and 1) != 0
        val vertical = (mode and 2) != 0
        if (!horizontal && !vertical) return 0
        val pixels = IntArray(surface.width * surface.height)
        var a = surface.bufferAddress
        for (i in pixels.indices) { pixels[i] = memory.read16(a); a += 2 }
        for (y in 0 until surface.height) {
            val sy = if (vertical) surface.height - 1 - y else y
            for (x in 0 until surface.width) {
                val sx = if (horizontal) surface.width - 1 - x else x
                val color = pixels[sy * surface.width + sx]
                memory.write16(surface.bufferAddress + (y * surface.width + x) * 2, color)
            }
        }
        return 0
    }
