...
class VxpSession(
    ...
    private val engine = VxpCoreEngine(storageRoot, textRasterizer, options.trace, audioHost)
    ...
    /** Forward host lifecycle changes without leaking platform types into vxp-core. */
    fun onHostPause() = audioHost.onHostPause()
    fun onHostResume() = audioHost.onHostResume()

    /** Host-facing playback diagnostics/control; guest ABI remains inside MreRuntime. */
...
object VxpCoreLibrary {
    ...
    fun open(
        ...
        audioHost: MreAudioHost = StateOnlyMreAudioHost()
    ): VxpSession = VxpSession(bytes, fileName, storageRoot, listener, options, textRasterizer, audioHost)
}
