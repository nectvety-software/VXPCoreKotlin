# VXP Core Library v0.8.2 — Kotlin-only
...
| Dạng VXP | Backend | Trạng thái v0.8.2 |
|---|---|---|
...
- Thumb ALU: `ADC`, `SBC`, `ROR`, `NEG`, `CMN`.
- Regression chính xác opcode `0x4241 = NEG r1,r0` gặp trong Spider-Man.
- `strtoi` là API Kotlin thật, không còn bị tính là generic stub.
- `vm_send_sms` bị sandbox: luôn trả failure, không gọi Android SMS/Intent/API mạng.
- Đồng bộ `AndroidVxpCore.VERSION` và `VxpCoreLibrary.VERSION` = `0.8.2`.

Xem `validation/` để biết kết quả chạy hai VXP người dùng đã cung cấp.
