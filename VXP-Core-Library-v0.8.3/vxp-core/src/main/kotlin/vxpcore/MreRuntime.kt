...
class MreRuntime(
    ...
    init {
        ...
        // Mediatek libc helper exported by many stripped ARMCC VXP builds.
        ...
        api("vm_midi_stop") { it.r[0] = 0 }
        api("vm_midi_get_time") { it.r[0] = 0 }
        api("vm_midi_play_by_bytes") { it.r[0] = 1 }
        api("vm_audio_play_bytes_no_block") { it.r[0] = 1 }
        api("vm_audio_stop_all") { it.r[0] = 0 }
        api("vm_audio_is_app_playing") { it.r[0] = 0 }
        ...
        api("vm_graphic_fill_rect_ex") { cpu ->
            // GCC MRE wrapper forwards five arguments: (layer, x, y, width, height).
        ...
        api("vm_graphic_line") { cpu ->
            // Buffer ABI used by GCC/MRE: (buffer, x1, y1, x2, y2 [, color]).
