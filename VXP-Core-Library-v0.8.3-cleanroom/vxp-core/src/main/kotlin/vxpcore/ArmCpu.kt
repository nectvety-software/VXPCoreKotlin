...
class ArmCpu(
    ...
    fun step() {
        ...
        try {
            ...
            val regs = buildString {
                for (i in 0..12) {
                    if (i > 0) append(' ')
                    append("r").append(i).append("=0x").append(r[i].toUInt().toString(16))
                }
                append(" sp=0x").append(r[13].toUInt().toString(16))
                append(" lr=0x").append(r[14].toUInt().toString(16))
            }
    ...
    private fun stepArm() {
        ...
        // ARMv5 CLZ (Count Leading Zeros). This must be decoded before the
        // generic data-processing block: its encoding overlaps that space and
        // treating it as ALU code silently corrupts capability/compatibility tests.
        if ((insn and 0x0fff0ff0) == 0x016f0f10) {
            val rd = (insn ushr 12) and 0xf
            val rm = insn and 0xf
            r[rd] = Integer.numberOfLeadingZeros(r[rm])
            return
        }
        ...
        if ((insn and 0x0c000000) == 0x04000000) {
            ...
            val rn = (insn ushr 16) and 0xf
            val rd = (insn ushr 12) and 0xf
            val offset = if (!i) insn and 0xfff else decodeShiftedRegister(insn and 0xfff, pc).first
        ...
        if ((insn and 0x0e000090) == 0x00000090 && ((insn ushr 5) and 0x3) != 0) {
            ...
            if (load) {
                ...
                when (sh) {
                    ...
                    2 -> { // LDRD: ARMv5 doubleword load, Rd and Rd+1
                        require(rd < 15) { "LDRD invalid Rd=r$rd" }
                        r[rd] = memory.read32(addr)
                        r[rd + 1] = memory.read32(addr + 4)
                    }
                    3 -> { // STRD: ARMv5 doubleword store, Rd and Rd+1
                        require(rd < 15) { "STRD invalid Rd=r$rd" }
                        memory.write32(addr, r[rd])
                        memory.write32(addr + 4, r[rd + 1])
                    }
                }
            }
        ...
        // ARM UMULL/UMLAL/SMULL/SMLAL (long multiply).
        // GCC/newlib printf uses these heavily for 64-bit integer conversion.
        // These encodings otherwise fall through into generic data-processing and
        // silently corrupt the quotient/digit loop.
        if ((insn and 0x0f8000f0) == 0x00800090) {
            val signed = (insn and (1 shl 22)) != 0
            val accumulate = (insn and (1 shl 21)) != 0
            val setFlags = (insn and (1 shl 20)) != 0
            val rdHi = (insn ushr 16) and 0xf
            val rdLo = (insn ushr 12) and 0xf
            val rs = (insn ushr 8) and 0xf
            val rm = insn and 0xf

            var result = if (signed) {
                r[rm].toLong() * r[rs].toLong()
            } else {
                r[rm].toUInt().toLong() * r[rs].toUInt().toLong()
            }
            if (accumulate) {
                val acc = (r[rdHi].toLong() shl 32) or r[rdLo].toUInt().toLong()
                result += acc
            }
            r[rdLo] = result.toInt()
        ...
        if ((insn and 0x0c000000) == 0x00000000) {
            ...
            val (op2, shCarry) = if (immediate) decodeImmediate(insn) else decodeShiftedRegister(insn and 0xfff, pc)
            when (opcode) {
    ...
    private fun stepThumb() {
        ...
        if ((op and 0xFC00) == 0x4400) {
            ...
            // In Thumb state, architectural reads of PC use address(current instruction)+4.
            // r15 internally points at pc+2 after fetch, so PIC sequences such as
            // `ldr r3, literal; add r3, pc` must not read r15 directly.
            val rsValue = if (rs == 15) pc + 4 else r[rs]
            val rdValue = if (rd == 15) pc + 4 else r[rd]
            when (kind) {
                0 -> {
                    val result = rdValue + rsValue
                    if (rd == 15) { thumb = true; r[15] = result and -2 } else r[rd] = result
                }
                1 -> {
                    val result = rdValue - rsValue
                    setNZ(result); c = carrySub(rdValue, rsValue); v = overflowSub(rdValue, rsValue, result)
                }
                2 -> {
                    val value = rsValue
                    if (rd == 15) { thumb = true; r[15] = value and -2 } else r[rd] = value
                }
                3 -> {
                    // ARMv5T Thumb high-register branch exchange: H1 distinguishes
                    // BX (H1=0) from BLX register (H1=1).
                    val link = h1 != 0
                    branchExchange(rsValue, link = link, returnAddress = pc + 2)
                }
            }
            return
        }
        ...
        // Halfword load/store with immediate offset (Thumb format 10).
        // 1000 L imm5 Rb Rd, byte offset = imm5 << 1.
        if ((op and 0xF000) == 0x8000) {
            val load = (op and 0x0800) != 0
            val imm = ((op ushr 6) and 0x1f) shl 1
            val rb = (op ushr 3) and 0x7
            val rd = op and 0x7
            val addr = r[rb] + imm
            if (load) r[rd] = memory.read16(addr) else memory.write16(addr, r[rd])
            return
        }
    ...
    private fun decodeShiftedRegister(op2: Int, instructionPc: Int): Pair<Int, Boolean> {
        val rm = op2 and 0xf
        // In ARM state, reading r15 as an operand yields address(current instruction)+8.
        // r15 internally points at pc+4 after fetch, so using r[15] here makes PIC/veneer
        // sequences such as `ADD pc, r12, pc` jump four bytes early. This broke the
        // GCC __aeabi_* interworking veneers used by Whisk3D and RetroMRE.
        val value = if (rm == 15) instructionPc + 8 else r[rm]
        if ((op2 and 0x10) != 0) {
            val rs = (op2 ushr 8) and 0xf
            val rsValue = if (rs == 15) instructionPc + 8 else r[rs]
            val amount = rsValue and 0xff
